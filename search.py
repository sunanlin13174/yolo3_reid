...
from reid import make_data_loader
from reid import build_transforms
from reid import build_model
from reid import reidCfg
...
def detect(cfg,
    ...
    feat2 = []
    l = 0
    L = len(os.listdir('./query'))
    for query_img in os.listdir('./query'):
        if query_img.split('_')[0]=='0001':
            l+=1
    print("0001 have {} pictures,and 0002 have {} pictures".format(l,L-l))
    for i, batch in enumerate(query_loader):
        with torch.no_grad():
            ...
            feat1=feat[0:l,:]
            feat2=feat[l:,:]
            query_feats1.append(feat1)
            query_feats2.append(feat2)
            query_pids.extend(np.asarray(pid))  # extend() 函数用于在列表末尾一次性追加另一个序列中的多个值（用新列表扩展原来的列表）。
    query_feats1 = torch.cat(query_feats1, dim=0)  # torch.Size([2, 2048])
    query_feats2 = torch.cat(query_feats2, dim=0)
    print(len(query_feats1),len(query_feats2))
    print("The query feature is normalized")
    query_feats1 = torch.nn.functional.normalize(query_feats1, dim=1, p=2) # 计算出查询图片的特征向量
    query_feats2 = torch.nn.functional.normalize(query_feats2,dim=1,p=2)
    ############# 行人检测模型初始化 #############
    ...
    for i, (path, img, im0, vid_cap) in enumerate(dataloader):
        ...
        if det is not None and len(det) > 0:
            ...
            if gallery_img:
                ...
                # m: 2
                # n: 7
                m,M,n = query_feats1.shape[0],query_feats2.shape[0], gallery_feats.shape[0]
                distmat1 = torch.pow(query_feats1, 2).sum(dim=1, keepdim=True).expand(m, n) + \
                          torch.pow(gallery_feats, 2).sum(dim=1, keepdim=True).expand(n, m).t()
                distmat2 = torch.pow(query_feats2, 2).sum(dim=1, keepdim=True).expand(M, n) + \
                           torch.pow(gallery_feats, 2).sum(dim=1, keepdim=True).expand(n, M).t()
                ...
                distmat1.addmm_(1, -2, query_feats1, gallery_feats.t())
                distmat2.addmm_(1,-2,query_feats2,gallery_feats.t())
                # distmat = (qf - gf)^2
                ...
                distmat1 = distmat1.cpu().numpy()  # <class 'tuple'>: (3, 12)
                distmat1 = distmat1.sum(axis=0) / len(query_feats1) # 平均一下query中同一行人的多个结果
                index1 = distmat1.argmin()
                if distmat1[index1] < dist_thres:
                    print('距离：%s'%distmat1[index1])
                    plot_one_box(gallery_loc[index1], im0, label='find_0001!', color=colors[int(cls)])
                    # cv2.imshow('person search', im0)
                    # cv2.waitKey()
                distmat2 = distmat2.cpu().numpy()  # <class 'tuple'>: (3, 12)
                distmat2 = distmat2.sum(axis=0) / len(query_feats2)  # 平均一下query中同一行人的多个结果
                index2 = distmat2.argmin()
                if distmat2[index2] < dist_thres:
                    print('距离：%s' % distmat2[index2])
